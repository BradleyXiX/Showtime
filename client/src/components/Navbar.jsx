  ...
  return (
    <div className="fixed top-0 left-0 w-full flex items-center justify-between px-6  md:px-16 lg:px-36 py-5 ">
      <Link to='/' className='max-md:flex-1'>
        <img src={assets.logo} alt="Showtime-logo" className="w-40 h-auto transition duration-300 ease-in-out hover:drop-shadow-[0_0_10px_#facc15]" />
      </Link>

      <div className={`max-md:absolute max-md:top-0 max-md:left-0 max-md:front-medium max-md:text-lg z-50 flex flex-col md:flex-row items-center max-md:justify-center gap-8 min-md:px-8 py-3 max-md:h-screen min-md:rounded-full backdrop-blur bg-black/70 md:bg-white/10 md:border border-gray-300/20 overflow-hidden transition-[width] duration-300 ${isOpen ? 'max-md:w-full' : 'max-md:w-0'}`}>
      ...
      <div className="flex items-center gap-8">
        ...
        {
          !user ? (
            <button onClick={openSignIn} className="px-4 py-1 sm:px-7 sm:py-2 bg-primary hover:bg-primary-dull hover:text-black duration-700 rounded-full font-medium cursor-pointer ">Login</button>
          ) : (
            <UserButton>
